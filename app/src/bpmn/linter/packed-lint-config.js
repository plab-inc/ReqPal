...
var global = function() {
  ...
  function getEvents(definition) {
    return definition.rootElements.filter(node => isAny$7(node, ["bpmn:Error", "bpmn:Escalation", "bpmn:Message", "bpmn:Signal"]));
  }
  ...
  function isReferenced(event, eventDefinitions) {
    if (is$g(event, "bpmn:Error")) {
      return (
        eventDefinitions.some(node => is$g(node, "bpmn:ErrorEventDefinition") && event.id === node.errorRef?.id)
      );
    }

    if (is$g(event, "bpmn:Escalation")) {
      return (
        eventDefinitions.some(node => is$g(node, "bpmn:EscalationEventDefinition") && event.id === node.escalationRef?.id)
      );
    }

    if (is$g(event, "bpmn:Message")) {
      return (
        eventDefinitions.some(node => is$g(node, "bpmn:MessageEventDefinition") && event.id === node.messageRef?.id)
      );
    }

    if (is$g(event, "bpmn:Signal")) {
      return (
        eventDefinitions.some(node => is$g(node, "bpmn:SignalEventDefinition") && event.id === node.signalRef?.id)
      );
    }
  }
  ...
  function isUnique(event, events) {
    return (
      events.filter(node => is$g(node, event.$type) && event.name === node.name).length === 1
    );
  }
};
...
function isLinkEvent(node) {
  ...
  return eventDefinitions.some(
    definition => is$e(definition, "bpmn:LinkEventDefinition")
  );
}
